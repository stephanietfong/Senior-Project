  ...
  useEffect(() => {
    const run = async () => {
      try {
        ...
        if (!authUser) {
          setErrorMsg(
            "You must be logged in to view the coordinator page."
          );
          setPastEvents([]);
          return;
        }
        ...
        const withCounts = await Promise.all(
          hostedPast.map(async (e: any) => {
            const count = await getRSVPCountForEvent(
              e.event_id
            );
            return { e, count };
          })
        ...
        setErrorMsg(
          err?.message ||
            "Failed to load coordinator view."
        );
      } finally {
        setLoading(false);
      }
      ...
      <div className="my-10 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="bg-customGreen py-2 font-semibold px-4 hover:opacity-90"
          >
            Create Event +
          </button>
