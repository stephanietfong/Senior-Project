...
export const MyEventsPage = () => {
  ...
  const handleHostedBack = () => {
    setHostedIndex((prev) =>
      hostedEvents.length === 0
        ? 0
        : (prev - 1 + hostedEvents.length) % hostedEvents.length,
    );
  };
  const handleHostedForward = () => {
    setHostedIndex((prev) =>
      hostedEvents.length === 0 ? 0 : (prev + 1) % hostedEvents.length,
    );
  };
  const handleAttendedBack = () => {
    setAttendedIndex((prev) =>
      attendedEvents.length === 0
        ? 0
        : (prev - 1 + attendedEvents.length) % attendedEvents.length,
    );
  };
  const handleAttendedForward = () => {
    setAttendedIndex((prev) =>
      attendedEvents.length === 0 ? 0 : (prev + 1) % attendedEvents.length,
    );
  };
  ...
  return (
    <>
      ...
      >
        <h1 className="subheader-text">Events Hosted by Me</h1>
        <div className="carousel">
          <button
            onClick={handleHostedBack}
            disabled={hostedEvents.length <= 1}
          >
            <img
              src={backArrow}
              alt="Back"
          ...
          {hostedEvents.length > 0 ? (
            <PastEventCard
              key={hostedEvents[hostedIndex].id}
              event={hostedEvents[hostedIndex]}
            />
          ) : (
            <span>No hosted events found.</span>
          )}
          <button
            onClick={handleHostedForward}
            disabled={hostedEvents.length <= 1}
          >
            <img
              src={forwardArrow}
