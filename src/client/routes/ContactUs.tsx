...
export function ContactUsPage() {
  ...
  const handleSubmit = async () => {
    ...
    if (!issueName.trim())
      return setErrorMsg("Please enter a name for the issue.");
    if (!issueDescription.trim())
      return setErrorMsg("Please describe the issue.");
  ...
  return (
    <div className="min-h-screen bg-customBeige text-black font-redhat">
      {/* Title + divider */}
      <div className="py-6 text-center">
        <h1 className="subheader-text">Contact Us</h1>
      </div>
      <hr className="border-black" />
      <div className="px-6 py-4">
        <button
          onClick={handleBack}
          className="bg-customGreen button-style top-6 left-6 flex items-center gap-1 text-sm font-medium text-black hover:opacity-60 transition-opacity cursor-pointer"
        >
          ← Back
        </button>
      </div>
      ...
      <div className="mx-auto max-w-3xl space-y-6 px-6 pb-10">
        ...
        <div className="rounded bg-gray-300 p-6 text-base leading-relaxed">
          ...
          <p className="mt-3">
            ...
            <a
              href="mailto:localloopLLC@gmail.com"
              className="underline hover:opacity-75"
            >
              localloop4914@gmail.com
            </a>
          </p>
        </div>

        {/* Report form box */}
        <div className="rounded bg-gray-300 p-6 font-redhat">
